		...
		if(value === undefined){
			...
			if(value !== null){
				if(value instanceof Array){
					if(schema.items){
						...
						for (var i = 0, l = value.length; i < l; i += 1) {
							...
							errors.push.apply(errors, checkProp(value[i],propDef,path,i));
						}
					}
				...
				if(schema.properties || schema.additionalProperties){
					errors.push.apply(errors, checkObj(value, schema.properties, path, schema.additionalProperties));
				}
	...
	function checkObj(instance,objTypeDef,path,additionalProp){
		...
		for(var i in instance){
			...
			if(!_changing && value && value.$schema){
				errors.push.apply(errors, checkProp(value,value.$schema,path,i));
			}
		}
		return errors;
	}
